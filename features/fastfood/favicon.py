    ...
    def get(self, domain):
        ...
        if response is not None:
            if response.status_code == 200:
                ...
                fihash = mmh3.hash(favicon)
                r = np.where(df['http.favicon.hash'] == fihash)
                try:
        ...
        return jsonify({"feature": "favicon", "domain": domain, "result": result, "detail": detail,
                        "meta": {"hash": fihash, "ssl": sslStatus}})
